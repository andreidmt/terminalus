 ...
 */
const _cmdStatus = cmdProc => cmdProc.pid ? cmdProc.pid : "ended";
...
const _bindCMDtoLog = (cmdProc, logBox) => {
    ...
    cmdProc.on("exit", (code, signal) => {
        ...
        logBox.log(logByType(`Ended with code: ${code}, signal: ${signal}`));
    });
};
...
const getCMDBox = command => {
    ...
    const processParams = [command.cmd, command.args || [], {
        cwd: process.cwd(),
        env: process.env,
        detatched: false,
        encoding: "utf8"
    ...
    const logBox = _blessed.log(Object.assign(M.clone(DEFAULT_BOX_OPTIONS), command.position, {
        label: command.title
    }));
     ...
     */
    logBox.on("focus", () => {
        logBox.setLabel(`## ${logBox.options.label}: ${_cmdStatus(cmdProcess)}`);
